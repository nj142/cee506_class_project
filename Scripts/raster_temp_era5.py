...
def mean_30d_raster_temps(bounding_box, breakup_anomaly_data, estimated_breakup_doy):
    # Convert average DOY to date
    estimated_breakup_date = datetime.date(list(breakup_anomaly_data.keys())[0], 1, 1) + datetime.timedelta(days=int(estimated_breakup_doy) - 1)
    ...
    request = {
        ...
        "year": list(breakup_anomaly_data.keys())[0],
